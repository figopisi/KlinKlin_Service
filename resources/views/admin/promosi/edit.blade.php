...
<style>
    ...
    *{box-sizing:border-box;}
    ...
    .status-pill.on{ background:var(--brand-tint); color:var(--brand-dark); }
    .status-pill.off{ background:var(--muted-tint); color:var(--ink-soft); }
    ...
    .usage-box .divider{ width:1px; background:#c7d9f2; }
...
<div class="wrap">
    ...
    <div class="page-header">
        <div>
            ...
            <p>Perbarui detail promo — perubahan langsung berlaku setelah disimpan.</p>
        </div>
        @if($promo->is_active)
            <span class="status-pill on"><span class="blip"></span>Sedang Aktif</span>
        @else
            <span class="status-pill off"><span class="blip"></span>Nonaktif</span>
        @endif
    </div>
    ...
    <form method="POST" action="{{ route('admin.promosi.update', $promo->id) }}" id="promoForm">
        @csrf
        @method('PUT')
            ...
            <div>
                <div class="card">
                    ...
                    <div class="field {{ $errors->has('nama_promo') ? 'error' : '' }}">
                        ...
                        <input type="text" name="nama_promo" id="input_nama"
                               ...
                               value="{{ old('nama_promo', $promo->nama_promo) }}" oninput="updatePreview()">
                    ...
                    <div class="field {{ $errors->has('deskripsi') ? 'error' : '' }}">
                        ...
                        <textarea name="deskripsi" id="input_deskripsi"
                                  ...
                                  oninput="updatePreview()">{{ old('deskripsi', $promo->deskripsi) }}</textarea>
                        ...
                        <div class="field {{ $errors->has('harga_awal') ? 'error' : '' }}">
                            ...
                            <div class="price-input-wrap">
                                ...
                                <input type="number" name="harga_awal" id="input_harga_awal"
                                       placeholder="0" min="0"
                                       value="{{ old('harga_awal', $promo->harga_awal) }}" oninput="updatePreview()">
                            </div>
                        ...
                        <div class="field {{ $errors->has('harga_promo') ? 'error' : '' }}">
                            ...
                            <div class="price-input-wrap">
                                ...
                                <input type="number" name="harga_promo" id="input_harga_promo"
                                       placeholder="0" min="0"
                                       value="{{ old('harga_promo', $promo->harga_promo) }}" oninput="updatePreview()">
                            </div>
                ...
                <div class="card">
                    ...
                    @if($promo->kuota !== null)
                        <div class="usage-box">
                            <div>
                                <div class="num">{{ $promo->terpakai }}</div>
                                <div class="lbl">TERPAKAI</div>
                            </div>
                            <div class="divider"></div>
                            <div>
                                <div class="num">{{ $promo->kuota - $promo->terpakai < 0 ? 0 : $promo->kuota - $promo->terpakai }}</div>
                                <div class="lbl">SISA KUOTA</div>
                            </div>
                        </div>
                    @endif
                    ...
                    <div class="row-2">
                        <div class="field {{ $errors->has('tanggal_mulai') ? 'error' : '' }}">
                            ...
                            <input type="datetime-local" name="tanggal_mulai"
                                   value="{{ old('tanggal_mulai', $promo->tanggal_mulai ? \Carbon\Carbon::parse($promo->tanggal_mulai)->format('Y-m-d\TH:i') : '') }}">
                        ...
                        <div class="field {{ $errors->has('tanggal_selesai') ? 'error' : '' }}">
                            ...
                            <input type="datetime-local" name="tanggal_selesai"
                                   value="{{ old('tanggal_selesai', $promo->tanggal_selesai ? \Carbon\Carbon::parse($promo->tanggal_selesai)->format('Y-m-d\TH:i') : '') }}">
                    ...
                    <div class="field" style="margin-top:16px;">
                        <div class="toggle-row">
                            <div>
                                <div class="txt">Status promo</div>
                                <div class="sub">Nonaktifkan untuk sembunyikan dari pelanggan tanpa menghapus data</div>
                            </div>
                            <label class="switch">
                                <input type="checkbox" name="is_active" value="1"
                                       {{ old('is_active', $promo->is_active) ? 'checked' : '' }}
                                       id="input_active" onchange="updatePreview()">
